...
class SingleTimeDet(model_base.ModelBase):
  """SingleTimeDet model to provide instance-level annotations. """

  def __init__(self, options, is_training):
    """Constructs the SingleTimeDet instance. """
    if not isinstance(options, model_pb2.SingleTimeDet):
      raise ValueError('Options has to be an SingleTimeDet proto.')
    super(SingleTimeDet, self).__init__(options, is_training)
  ...
  def predict(self, inputs, **kwargs):
    ...
    video_masks = tf.sequence_mask(video_lengths, 
                                   maxlen=tf.shape(video_seq_features)[1],
                                   dtype=tf.float32)
    video_features = masked_ops.masked_avg_nd(video_seq_features, video_masks, dim=1)
    video_features = tf.squeeze(video_features, 1)
  ...
  def build_losses(self, predictions, labels, **kwargs):
    ...
    batch = noun_logits.shape[0].value
