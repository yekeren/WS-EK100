...
class SingleTimeClassAgnosticDet(model_base.ModelBase):
  """SingleTimeClassAgnosticDet model to provide instance-level annotations. """

  def __init__(self, options, is_training):
    """Constructs the SingleTimeClassAgnosticDet instance. """
    if not isinstance(options, model_pb2.SingleTimeClassAgnosticDet):
      raise ValueError('Options has to be an SingleTimeClassAgnosticDet proto.')
    super(SingleTimeClassAgnosticDet, self).__init__(options, is_training)
  ...
  def _compute_class_agnostic_attention(self,
                                     video_features,
                                     video_lengths,
                                     n_classes=None,
    ...
    Returns:
      attention: A [batch, max_video_length] tensor.
      pre_attention: A [batch, max_video_length] tensor.
    """
    dims = video_features.shape[-1].value
    max_video_length = tf.shape(video_features)[1]

    # Compute attention masks.
    #   masks shape = [batch, max_max_video_length].
    ...
    # Compute attention.
    #   pre_attention shape = [batch, max_max_video_length].
    #   attention shape = [batch, max_max_video_length].
    ...
    if not use_gumbel_softmax:
      ...
      pre_attention = tf.squeeze(pre_attention, -1)
    ...
    l1_loss = tf.div(tf.reduce_sum(attention * masks), tf.reduce_sum(masks))
  ...
  def predict(self, inputs, **kwargs):
    ...
    # Compute attention.
    verb_attention, _ = self._compute_class_agnostic_attention(
        video_seq_features, 
        video_lengths, 
        ...
        name='VerbAttention')
    noun_attention, _ = self._compute_class_agnostic_attention(
        video_seq_features, 
        video_lengths, 
        use_gumbel_softmax=options.use_gumbel_softmax,
        gumbel_softmax_temperature=options.gumbel_softmax_temperature,
