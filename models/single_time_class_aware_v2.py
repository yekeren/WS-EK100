...
class SingleTimeClassAwareDetV2(model_base.ModelBase):
  """SingleTimeClassAwareDetV2 model to provide instance-level annotations. """

  def __init__(self, options, is_training):
    """Constructs the SingleTimeClassAwareDetV2 instance. """
    if not isinstance(options, model_pb2.SingleTimeClassAwareDetV2):
      raise ValueError('Options has to be an SingleTimeClassAwareDetV2 proto.')
    super(SingleTimeClassAwareDetV2, self).__init__(options, is_training)
  ...
  def predict(self, inputs, **kwargs):
    ...
    # Apply attention.
    #   verb_logits shape = [batch, n_verb_classes, n_verb_classes].
    #   noun_logits shape = [batch, n_noun_classes, n_verb_classes].
    verb_logits = tf.einsum('bcl,bld->bcd', verb_attention, verb_seq_classification)
    noun_logits = tf.einsum('bcl,bld->bcd', noun_attention, noun_seq_classification)
    verb_logits = tf.math.divide(verb_logits, 1e-10 + tf.reduce_sum(verb_attention, -1, keepdims=True))
    noun_logits = tf.math.divide(noun_logits, 1e-10 + tf.reduce_sum(noun_attention, -1, keepdims=True))
  ...
  def build_losses(self, predictions, labels, **kwargs):
    ...
    noun_logits = tf.gather_nd(noun_logits, 
                               tf.stack([batch_id, noun_ids], -1))
    verb_logits = tf.gather_nd(verb_logits, 
  ...
  def build_metrics(self, predictions, labels, **kwargs):
    ...
    #verb_scores = verb_scores * tf.transpose(verb_seq_attention, [0, 2, 1])
    # verb_scores = tf.transpose(verb_seq_attention, [0, 2, 1])
