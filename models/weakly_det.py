...
class WeaklyDet(model_base.ModelBase):
  """WeaklyDet model to provide instance-level annotations. """

  def __init__(self, options, is_training):
    """Constructs the WeaklyDet instance. """
    if not isinstance(options, model_pb2.WeaklyDet):
      raise ValueError('Options has to be an WeaklyDet proto.')
    super(WeaklyDet, self).__init__(options, is_training)
  ...
  def _compute_class_aware_attention(self,
    ...
    if not use_gumbel_softmax:
      ...
      attention = gumbel_softmax(pre_attention, temperature=gumbel_softmax_temperature, hard=True)
  ...
  def predict(self, inputs, **kwargs):
    ...
    verb_feature = tf.math.divide(verb_feature, 1e-12 + tf.reduce_sum(verb_attention, -1, keepdims=True))
    noun_feature = tf.math.divide(noun_feature, 1e-12 + tf.reduce_sum(noun_attention, -1, keepdims=True))
  ...
  def build_losses(self, predictions, labels, **kwargs):
    ...
    verb_losses = tf.nn.sigmoid_cross_entropy_with_logits(
        labels=verb_grid_labels, logits=verb_logits)
    noun_losses = tf.nn.sigmoid_cross_entropy_with_logits(
        labels=noun_grid_labels, logits=noun_logits)

    def reduce_mean(losses, masks):
      return tf.math.divide(tf.reduce_sum(losses * tf.expand_dims(masks, 2)), 
  ...
  def build_metrics(self, predictions, labels, **kwargs):
    ...
    # Post-processing.
    verb_scores = tf.nn.sigmoid(verb_seq_classification)
